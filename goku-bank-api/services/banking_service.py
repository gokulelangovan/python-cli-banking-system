...
class BankingService:
    ...
    def transfer(self, sender_acc_no, receiver_acc_no, amount):
        ...
        try:
            ...
            conn.execute(
                """
                INSERT INTO transactions(account_id, transaction_type, amount, reference)
                VALUES (?, ?, ?, ?)
                """,
                (sender["id"], "TRANSFER_OUT", amount, f"Transfer to {receiver_acc_no}")
            )
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?)
                """,
                (receiver["id"], "TRANSFER_IN", amount, f"Transfer from {sender_acc_no}")
            )
